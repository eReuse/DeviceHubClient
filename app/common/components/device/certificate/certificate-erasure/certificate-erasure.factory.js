...
function certificateErasureFactory (CONSTANTS, CERTIFICATE_ERASURE_FACTORY_STRINGS, ResourceSettings, session) {
  ...
  function certificateErasure (computers, model, logo) {
    var self = this
    computers = _.cloneDeep(computers)
    this.model = model
    this.logo = logo
    ...
    this.pdf = {
      ...
      info: {
        title: 'Certificate of erasure',
        author: model.org || CONSTANTS.appName,
        keywords: 'report,certificate,hard-drive,eReuse.org,' + CONSTANTS.appName
      },
      styles: {
        h1: {
          fontSize: 22,
  ...
  proto.generatePdf = function () {
    ...
    this.promise.then(function () {
      ...
      self.pdf.content = self.content
      pdfMake.createPdf(self.pdf).open()
    })
  }
